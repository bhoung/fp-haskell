...
filter f = foldRight (\x xs -> if f x == True then x :. xs else xs) Nil
...
--flatten ((x) :. Nil) = x
--flatten ((x :. Nil) :. (y :.Nil) :. Nil) = (x :. y :. Nil)
...
--lengthGT4 xs = if length xs > 3 then True else False
