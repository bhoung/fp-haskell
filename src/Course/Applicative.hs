...
instance Applicative Optional where
  ...
  (<*>) fo fa = bindOptional (\f -> mapOptional f fa) fo
...
--filtering p xs =
--foldRight (\x fxs -> lift2 (\b xs' -> if b then x :. xs' else xs') (p x) fxs) (pure Nil) xs
--filtering p xs = 
--foldRight (\a fxs -> lift2 (\b xs' -> if b then a :. xs' else xs') (p a) fxs) (pure Nil) xs

filtering p xs = foldRight (\a fxs -> lift2 (f a) (p a) fxs) (pure Nil) xs
     where f a b xs' = if b then a :. xs' else xs' 

--filtering p xs = foldRight (\a fxs -> lift2 (\b xs' -> _todo) (pure Nil) xs
-----------------------
-- SUPPORT LIBRARIES --
-----------------------
